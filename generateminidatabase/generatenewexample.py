#
#   This code exists because the database that I am using is not perfectly clean. There are examples which do not contain ChatGPT answers or some other problems.
...
def generateexample():                                  # a function that I used for debugging, returns a 4-tuple with question,human answer, chatgpt answer and topic
    questionlist = generatequestionlist()               # not necessary for display version of the project
...
class questionanswer:                                               # a class I use to save every example
    def __init__(self, question, answerHUMAN, answerCGPT, topic):
        ...
        self.topic = topic
def generatequestionlist(): # a function that generates a list of questionanswer class instances from every line in the big dataset file "all.json"
